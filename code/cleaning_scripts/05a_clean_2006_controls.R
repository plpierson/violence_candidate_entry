...
# employment
employ_mun <- read.csv(here("data", "raw_data", "control_variables", "census_2007_unemployment_mun.csv"))  
...
employ_mun <- employ_mun %>%    
  mutate(unemploy_rate = (Unemployed) / (Total - Not.economically.active))


# first language
lang_mun <- read.csv(here("data", "raw_data", "control_variables", "census_2007_language_mun.csv"))  
...
# main dwelling type
dwell_mun <- read.csv(here("data", "raw_data", "control_variables", "census_2007_dwelling_mun.csv")) 
...
dwell_mun <- dwell_mun %>% 
  mutate(prop_informal = (Informal.dwelling.shack.in.backyard + Informal.dwelling.shack.NOT.in.backyard) / Total)


#population groups
pop_mun <- read.csv(here("data", "raw_data", "control_variables", "census_2007_ethnicity_mun.csv"))   
...
# merge
controls_mun_2006 <- left_join(mun_2011, df_controls_mun, by = "local_municipality_id")
...
# clean GIS geometries to prepare for merge
controls_mun_2006 <- st_as_sf(controls_mun_2006)
mun_2016 <- st_as_sf(mun_2016)

controls_mun_2006 <- st_make_valid(controls_mun_2006)
mun_2016 <- st_make_valid(mun_2016)

# areal interpolation to 2016 ward boundaries
df_areal <- aw_interpolate(mun_2016, 
                           tid = "local_municipality_id", 
                           source = controls_mun_2006,
...
# EXPORT DATA TO PROCESSED_DATA SUBDIRECTORY 
saveRDS(df_areal_2, here("data", "processed_data", "data_2006_controls_MUN.rds"))
